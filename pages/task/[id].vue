  ...
  <AppLayout>
    <template #header-title>
      <div class="flex items-center gap-2 text-sm text-surface-500">
        <NuxtLink to="/dashboard" class="hover:text-surface-200 transition-colors">Dashboard</NuxtLink>
        <ChevronRight :size="14" />
        <span class="text-surface-100 font-semibold truncate max-w-xs">{{ task?.title || 'Task' }}</span>
      </div>
    </template>
    ...
    <div v-else-if="!task" class="text-center py-24">
      ...
      <NuxtLink to="/dashboard" class="btn-primary mt-4 inline-flex">Back to Dashboard</NuxtLink>
    </div>
    ...
    <div v-else class="max-w-4xl mx-auto space-y-5">
      ...
      <div class="bg-surface-800 border border-surface-700 rounded-xl p-6">
        ...
        <div class="flex items-start gap-4 mb-5">
          <div class="flex-1">
            <div v-if="!editingTitle" class="group flex items-start gap-2">
              <h1 class="text-xl font-bold text-surface-50 leading-tight">{{ task.title }}</h1>
              <button @click="startEditTitle" class="opacity-0 group-hover:opacity-100 p-1 rounded hover:bg-surface-700 text-surface-500 transition-all mt-0.5 shrink-0">
                <Pencil :size="13" />
              </button>
            </div>
          ...
          <div class="flex items-center gap-2 shrink-0">
            <button @click="confirmDeleteTask" class="btn-ghost py-1.5 text-surface-500 hover:text-red-400 hover:bg-red-900/20 text-xs gap-1.5">
              <Trash2 :size="14" />
              Delete
            </button>
          </div>
        ...
        <div class="flex items-center gap-2 flex-wrap mb-5">
          ...
          <div class="relative" ref="priorityRef">
            ...
            <div v-if="showPriorityPicker" class="absolute top-8 left-0 bg-surface-800 border border-surface-700 rounded-xl shadow-dropdown z-30 overflow-hidden animate-fade-in min-w-[120px]">
              <button
                v-for="p in PRIORITIES" :key="p"
                @click="updateField('priority', p); showPriorityPicker = false"
                :class="['w-full flex items-center gap-2 px-3 py-2 text-xs hover:bg-surface-700 transition-colors text-surface-200', p === task.priority ? 'font-semibold' : '']"
              >
          ...
          <div class="relative" ref="statusRef">
            ...
            <div v-if="showStatusPicker" class="absolute top-8 left-0 bg-surface-800 border border-surface-700 rounded-xl shadow-dropdown z-30 overflow-hidden animate-fade-in min-w-[140px]">
              <button
                v-for="s in STATUSES" :key="s"
                @click="updateField('status', s); showStatusPicker = false"
                :class="['w-full flex items-center gap-2 px-3 py-2 text-xs hover:bg-surface-700 transition-colors text-surface-200', s === task.status ? 'font-semibold' : '']"
              >
          ...
          <span class="text-xs text-surface-500">Created {{ timeAgo(task.createdAt) }}</span>
          <span v-if="task.lastUpdatedBy" class="text-xs text-surface-500">· Updated by <span class="font-medium text-surface-400">{{ task.lastUpdatedBy }}</span></span>
        </div>
        ...
        <div class="mb-0">
          <div v-if="!editingDesc" class="group">
            <div class="flex items-start gap-2 mb-1">
              ...
              <button @click="startEditDesc" class="opacity-0 group-hover:opacity-100 p-0.5 rounded hover:bg-surface-700 text-surface-500 transition-all">
                <Pencil :size="11" />
              </button>
            </div>
            <p v-if="task.description" class="text-sm text-surface-300 leading-relaxed whitespace-pre-wrap">{{ task.description }}</p>
            <p v-else class="text-sm text-surface-600 italic">No description provided. Click to add one.</p>
          </div>
          <div v-else>
      ...
      <div class="grid grid-cols-3 gap-5">
          ...
          <div class="space-y-3">
            <div class="flex items-center justify-between">
              ...
              <div class="flex items-center gap-2">
                ...
                <span class="text-sm font-medium text-surface-200">{{ task.assignedToName }}</span>
              </div>
            </div>
            <div class="flex items-center justify-between">
              ...
              <div class="flex items-center gap-2">
                ...
                <span class="text-sm font-medium text-surface-200">{{ task.assignedByName }}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <!-- Dates -->
        <div class="bg-surface-800 border border-surface-700 rounded-xl p-5">
          <h3 class="text-xs font-semibold text-surface-500 uppercase tracking-wide mb-3">Dates</h3>
          <div class="space-y-3">
            <div class="flex items-center justify-between">
              <span class="text-xs text-surface-500">Created</span>
              <span class="text-sm font-medium text-surface-200">{{ formatDate(task.createdAt) }}</span>
            </div>
            <div class="flex items-center justify-between">
              <span class="text-xs text-surface-500">Due date</span>
              <div v-if="!editingDue" class="flex items-center gap-1.5 group">
                <span :class="['text-sm font-medium', task.dueDate ? (isOverdue ? 'text-red-400' : 'text-surface-200') : 'text-surface-600']">
                  {{ task.dueDate ? formatDate(task.dueDate) : 'Not set' }}
                </span>
                <button @click="editingDue = true; editDue = task.dueDate || ''" class="opacity-0 group-hover:opacity-100 p-0.5 rounded hover:bg-surface-700 text-surface-500 transition-all">
                  <Pencil :size="11" />
                </button>
              </div>
