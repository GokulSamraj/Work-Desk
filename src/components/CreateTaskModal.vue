  ...
  <Teleport to="body">
    <div class="modal-overlay" @click.self="$emit('close')">
      <div class="modal-content max-w-lg w-full">
        <!-- Header -->
        <div class="px-6 py-5 border-b border-surface-100 flex items-center justify-between sticky top-0 bg-white z-10">
          <div class="flex items-center gap-3">
            <div class="w-8 h-8 rounded-lg bg-brand-100 flex items-center justify-center">
              <Plus :size="16" class="text-brand-600" />
            </div>
            <h2 class="font-bold text-surface-900">Create New Task</h2>
          </div>
          <button @click="$emit('close')" class="p-1.5 rounded-lg hover:bg-surface-100 text-surface-400 transition-colors">
            <X :size="18" />
          </button>
        </div>
        ...
        <form @submit.prevent="handleSubmit" class="p-6 space-y-4">
          ...
          <div class="flex items-center gap-2 p-3 bg-surface-50 rounded-lg border border-surface-100">
            ...
            <span class="text-xs text-surface-500">Assigned by <strong class="text-surface-700">{{ authStore.user?.name }}</strong> (you)</span>
          </div>
...
import { createTask, createNotification, sendTaskAssignmentEmail } from '@/firebase/firestore'
