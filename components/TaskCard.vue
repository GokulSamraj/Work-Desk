<template>
  ...
  >
    ...
    <div class="flex items-start justify-between gap-2 mb-3">
      <h3 class="text-sm font-semibold text-surface-100 leading-snug line-clamp-2 flex-1">{{ task.title }}</h3>
    ...
    <!-- Description -->
    <p v-if="task.description" class="text-xs text-surface-400 line-clamp-2 mb-3">{{ task.description }}</p>
    ...
    <div
      :class="[
        ...
        isRunning ? 'bg-emerald-50 border border-emerald-200' : 'bg-surface-950 border border-surface-700'
      ]"
      @click.stop
    >
      <div class="flex items-center gap-2">
        ...
        <span :class="['font-mono text-xs font-medium tabular-nums', isRunning ? 'text-emerald-700' : 'text-surface-600']">
          {{ formatTimer(displayTime) }}
        </span>
        <span v-if="isRunning" class="text-[10px] font-semibold text-emerald-600 animate-timer-tick">LIVE</span>
      ...
      <div v-if="authStore.user?.role === 'admin' || task.assignedTo === authStore.user?.uid" class="flex items-center gap-1">
        ...
        <button
          ...
          class="w-6 h-6 rounded-md bg-surface-700 hover:bg-red-100 hover:text-red-600 text-surface-200 flex items-center justify-center transition-colors"
          title="Reset timer"
        >
    ...
    <div class="flex items-center justify-between">
      ...
      <div class="flex items-center gap-2">
        ...
        <span class="text-xs text-surface-400 truncate max-w-[80px]">{{ task.assignedToName }}</span>
      </div>
